...
import {
  ...
  checkCollisionZones,
} from "../../modules/interactions/handleCollisionsZones";
...
const Game = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { container: alertsContainer, draw: showAlert } = CreateAlerts(
        app.screen.width,
        app.screen.height
      );
      const alerts = {
      ...
      const focus = CreateFogOfWar(
        app.screen.width,
        app.screen.height,
        app.renderer
      );
      const controlCircle = CreateControlCircle(
        Math.max(app.screen.width / 10, 200),
        Math.max(app.screen.height - app.screen.height / 4, 300),
        150
      );

      const gameUi = createGameUI(app.screen.width, app.screen.height, [
        textureUIuse,
        textureUImap,
        textureUISettings,
        textureAttack,
      ...
      let defaultCirleCoords = {
        x: controlCircle.getChildByLabel("grey").x,
        y: controlCircle.getChildByLabel("grey").y,
      };
      ...
      app.ticker.add((time) => {
        ...
        const animationState = isMoving ? "run" : "idle";
        const shouldSend = isMoving || (!isMoving && !hasSentIdle);
        map.getChildByLabel(
          "player"
        ).children[1].text = `x: ${player.x} y: ${player.y} name: ${name}`;
        ...
        if (name && shouldSend) {
          ...
          // Обновляем флаги
          if (isMoving) {
            hasSentIdle = false; // Сброс при движении
          } else {
            hasSentIdle = true; // Запоминаем что отправили idle
          }
        }
      });
    })();
